...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i in range(start_layer, output_layer):
            ...
            hidden_stats_list.append(hidden_states)
            # print('hdshape',hidden_states.shape,layer_outputs[1].shape)
            if use_cache:
                next_decoder_cache += (layer_outputs[-1],)
            if output_attentions:
                all_self_attentions = all_self_attentions + (layer_outputs[1],)
        # print('ahd',all_hidden_states,return_dict)
        # exit()
        if output_hidden_states:
            all_hidden_states = all_hidden_states + (hidden_states,)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if encoder_embeds is None:
            # print('embed_input',input_ids.device,past_key_values_length,position_ids,token_type_ids,inputs_embeds)
            ...
            # exit()
            # print(self.embeddings,input_ids,past_key_values_length,embedding_output.shape)
            # exit()
        else:
            embedding_output = encoder_embeds
        # print('bert_embedding',embedding_output.shape,mode)
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        # print('sq_shape',sequence_output.shape,input_ids.shape,encoder_hidden_states.shape)
        ...
        if labels is not None:
            ...
            # print(labels.view(-1).shape,shifted_prediction_scores.shape,labels,prediction_scores.shape)
            # exit()
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # print(encoder_embeds)
        # exit()
        # print('embed_input', encoder_embeds)

        outputs, txt_feats_list = self.bert(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
        ...
        prediction_scores = self.cls(sequence_output)
        # print('pred_scores',prediction_scores.shape,labels)
        # exit()
        ...
        # print(outputs.hidden_states.shape,outputs.attentions.shape)
        ...
        return MaskedLMOutput(
            ...
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        ), txt_feats_list, prediction_scores
