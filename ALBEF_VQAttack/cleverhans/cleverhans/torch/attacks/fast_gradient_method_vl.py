...
def fast_gradient_method(
    ...
    y[1]=y[1][:,:feat_len,:]
    feat_len_txt = min(out[0].shape[1], y[0].shape[1])
    out[0] = out[0][:, :feat_len_txt, :]
    y[0] = y[0][:, :feat_len_txt, :]

    if ls==1:
        loss=(torch.sum((torch.sum(-cos1(out[0], y[0]), 1) + torch.sum(-cos1(out[1], y[1]), 1))))
    elif ls==0:
        loss = (
