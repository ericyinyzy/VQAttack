...
from cleverhans.torch.attacks.fast_gradient_method_vl import fast_gradient_method
...
def projected_gradient_descent(
    ...
    if rand_init:
        if rand_minmax is None:
            rand_minmax = eps
        eta = torch.zeros_like(x[0]).uniform_(-rand_minmax, rand_minmax)

    else:
        eta = torch.zeros_like(x[0])
    ...
    text_embed_gradient=None
    if ls==1:
        while i < nb_iter:
            # print('min_max_ori1', torch.min(ori_x), torch.max(ori_x))
            adv_x,text_embed_gradient = fast_gradient_method(
                model_fn,
                [adv_x,x[1]],
                eps_iter,
                norm,
    ...
    if norm == np.inf and clip_min is not None:
        # print('eps',eps + clip_min <= clip_max,eps_iter <= eps)
    ...
    return adv_x,text_embed_gradient
