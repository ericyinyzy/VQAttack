...
def config():
    ...
    # Transformer Setting
    model_arch = "vlmo_base_patch16" #vlmo_large_patch16
    drop_path_rate = 0.1
    ...
    data_root = "set the VQA_arrow path"
    #e.g."/data/ziyi/vlmo_data/VQA_arrow"
    ...
    pretrain_path='set the pretrain model path'
    #e.g./data/ziyi/vlmo_model/vlmo_base_patch16_224.pt

    load_path = "set the vqa model path"
    # e.g. data/ziyi/vlmo_model/vlmo_base_patch16_480_vqa.pt
...
def task_mlm_itm_itc_base():
    ...
    datasets = ["vqa"]
    loss_names = _loss_names({"itm": 0, "mlm": 1, "itc": 0,"vqa": 1})
    batch_size = 1024
    whole_word_masking = True
    learning_rate = 2e-4
    train_transform_keys = ["square_transform_randaug"]
...
def task_mlm_itm():
    exp_name = "mlm_itm"
    datasets = ["vqa"]
    loss_names = _loss_names({"itm": 1,"vqa": 1,"mlm": 1})
