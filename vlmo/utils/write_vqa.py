...
def make_arrow(root, dataset_root=None):
    ...
    for split, questions in zip(
        ...
        for q in tqdm(questions):
            # print(type(q["image_id"]),type(q["question_id"]),q["question"])
            # exit()
    ...
    for split, annots in zip(
        ...
        for q in tqdm(annots):
            # print(q)
            # print(q["multiple_choice_answer"])
            # exit()
...
make_arrow(root='/data/ziyi/MMA/data/VQAv2/')